...
func TestStore_GetTrace(t *testing.T) {
	...
	assert.Equal(t, "attr-1", got.Spans[0].Tags[0].Key)
	assert.Equal(t, "value-1", got.Spans[0].Tags[0].Value())
	assert.Equal(t, "attr-2", got.Spans[0].Tags[1].Key)
	assert.Equal(t, "value-2", got.Spans[0].Tags[1].Value())
